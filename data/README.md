...
| `raw/districts.geojson`, `raw/districts_inspect.json` | `scripts/02_fetch_districts.py` | OpenStreetMap via Overpass API `https://overpass-api.de/api/interpreter`, `boundary=administrative` relations | © OpenStreetMap contributors, ODbL 1.0 |
...
The first session could not reach the data hosts: `earth-search.aws.element84.com` and
`sentinel-cogs.s3.us-west-2.amazonaws.com` were blocked by the session's egress policy, and
`overpass-api.de` reset connections. The fetch scripts are written but have not yet run against
the live services. The rest of the pipeline was dry-run end to end on synthetic data
(`S3CASE_SYNTHETIC=1`).
